...
export default function ImeiTrabajador() {
  ...
  const parseImeiInfo = (input: string) => {
    ...
    lines.forEach((line, index) => {
      ...
      } else if (lowerLine.includes('imei:')) {
      ...
      } else if (line.includes('Origen:') && !line.includes('Origen:')) { // Fallback para Origen
        result.serial = line.substring(6).trim()
      }
      
      // Formato 2: Líneas sin etiquetas
      // Si es la primera línea y no contiene IMEI, podría ser marca/modelo
      else if (index === 0 && !extractImei(cleanLine) && cleanLine.length > 2) {
        if (!result.modelo_descripcion) {
          result.modelo_descripcion = cleanLine
        }
      }
      
      // Líneas que empiecen con marca común de teléfonos
      else if (/^(samsung|iphone|huawei|xiaomi|honor|oppo|vivo|realme|infinix|tecno|itel|nokia)/i.test(cleanLine)) {
        if (!result.modelo_descripcion) {
          result.modelo_descripcion = cleanLine
        }
      }
...
IMEI NO SE ENCUENTRA EN LISTA BLANCA✅`
  ...
  const getStatusColor = (estado: string | null) => {
    switch (estado) {
      case 'apto': return 'bg-emerald-100 text-emerald-800'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">📱 IMEI Justificado</h1>
          <p className="text-slate-600">Gestión de dispositivos procesados con verificación OSIPTEL</p>
        </div>
        <button
          onClick={() => {
            setEditingImei(null)
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-slate-500 focus:border-slate-500 text-slate-900"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Total', value: imeis.length, color: 'bg-slate-500' },
          { label: 'Aptos', value: imeis.filter(i => i.estado_osiptel === 'apto').length, color: 'bg-emerald-500' },
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    {imei.estado_osiptel !== 'pendiente' && (
                      <a
                        ...
                        className="text-slate-600 hover:text-slate-800 text-xs flex items-center mt-1"
                      >
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div>
                ...
                <textarea
                  ...
                  rows={4}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-slate-500 focus:border-slate-500 font-mono text-sm text-slate-900 placeholder-slate-500"
                  placeholder="Pega la información del dispositivo:&#10;&#10;Marca: Honor&#10;Nombre del tlf: X7B&#10;Imei: 864414074675206&#10;Modelo: CLK-LX3&#10;Origen: AM4U9X4904G06824&#10;&#10;o formato simple:&#10;Samsung S24 ultra&#10;351123965542967&#10;SM-S928BZKKTPA&#10;Serie RFCXA1533XA"
                />
                <button
                  type="button"
                  onClick={handleParseInfo}
                  className="mt-2 bg-slate-100 text-slate-700 px-4 py-2 rounded-lg hover:bg-slate-200 text-sm font-medium transition-colors"
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    {...register('imei')}
                    type="text"
                    className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-slate-500 focus:border-slate-500 text-slate-900 placeholder-slate-500"
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-slate-500 focus:border-slate-500 text-slate-900"
                >
              ...
              <div className="flex space-x-3 pt-4">
                <button
                  type="submit"
                  className="flex-1 bg-slate-600 text-white py-3 px-4 rounded-lg hover:bg-slate-700 font-medium transition-colors"
                >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <div className="bg-slate-50 p-4 rounded-lg mb-6">
              <h4 className="font-medium text-slate-900 mb-2">IMEI a verificar:</h4>
              <p className="text-slate-800 font-mono">{pendingImeiData.imei}</p>
            ...
            <div className="space-y-4">
              ...
              <div className="flex space-x-3">
                <button
                  ...
                  className="flex-1 bg-emerald-600 text-white py-3 px-4 rounded-lg hover:bg-emerald-700 font-medium transition-colors flex items-center justify-center space-x-2"
                >
              ...
              <button
                ...
                className="w-full bg-slate-300 text-slate-700 py-2 px-4 rounded-lg hover:bg-slate-400 transition-colors"
              >
